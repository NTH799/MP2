...
//define the connection and create the prefixes for the commands you use in the client
const (
	CONN_PORT = ":3333"
	CONN_TYPE = "tcp"
//max client set to 2 have tested over 10 and has crashed my goland. kept it at 2 for private messaging
	MAX_CLIENTS = 2
...
// Lobby struct handles all the various elements of lobbies when they are created
type Lobby struct {
	clients   []*Client
...
// Starts a new goroutine that listens through the lobby
...
// This function looks to see if there is an inactive chat room that needs to be deleted
...
func (lobby *Lobby) Help(client *Client) {
	...
	client.outgoing <- "/create chatroom - creates a chat room named chatroom\n"
	client.outgoing <- "/join chatroom - joins a chat room named chatroom\n"
	client.outgoing <- "/leave - leaves the current chat room\n"
	client.outgoing <- "/name Nick - changes your name to Nick\n"
	client.outgoing <- "/quit - quits the program\n"
	client.outgoing <- "\n"
	log.Println("client requested help")
}

// A ChatRoom struct to define the contents (name, clients within room, messages sent in room, and expiry time if necessary)
...
// Creates an empty chat room with the given name, and sets its expiry time to the current time + EXPIRY_TIME.
...
// Adds the given Client to the ChatRoom, and sends them all messages that have that have been sent since the creation of the ChatRoom.
